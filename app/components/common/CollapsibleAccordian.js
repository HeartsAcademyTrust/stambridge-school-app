...
class CollapsibleAccordian extends React.Component {
  ...
  render() {
    const listItem = this.props.data.map((item, index) => {
      let collapsibleHeaderClass = 'collapsible-header';
      let showUploadedDate = <span className="grey-text lighten-2">Uploaded: {moment(item[this.props.date_published_key]).format('Do MMMM YYYY')}</span>;
      let deadlineDateMobile = '';

      if (this.props.deadline_date) {
        const deadline = moment(item[this.props.date_published_key]);
        let collapsibleHeaderClassColour = ' green-text text-darken-4';
        ...
        deadlineDateMobile =
            ...
            <div className={`col-xs-12 hidden-sm-up ${collapsibleHeaderClassColour}`}>
              <i className="material-icons" style={{ marginRight: 5, paddingTop: '5px' }}>
                schedule
              </i>
