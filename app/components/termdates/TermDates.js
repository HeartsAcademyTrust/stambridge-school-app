...
class TermDates extends React.Component {
  ...
  componentDidMount() {
    helpers.getTermDates.then((response) => {
      this.setState({
  ...
  render() {
    const terms = this.state.term_dates.map((term, index, termDates) => {
      const date = moment(term.start_time);
      let imgFile = '';
      ...
      const midTerms = termDates.filter((termDate) => (
        moment(termDate.start_time).isBetween(moment(term.start_time), moment(term.end_time))
      ));

      if (midTerms.length > 0) {
        const midTermDates = midTerms.map((midTerm) => (
          <p>
            {midTerm.name} :
            {moment(midTerm.start_time).format('dddd, MMMM Do YYYY')} -
            {moment(midTerm.end_time).format('dddd, MMMM Do YYYY')}
          </p>
        ));

        const termStart = moment(term.start_time);
        return (
          <div className="card" style={seasonStyle} index={index}>
            <div className="card-content white-text" style={seasonStyle}>
              ...
              <p className="flow-text">
                {termStart.format('dddd, MMMM Do YYYY')} -
                {moment(term.end_time).format('dddd, MMMM Do YYYY')}
              </p>
              {midTermDates}
            </div>
          </div>
